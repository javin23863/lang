...
- Apple defines `Product Interaction` as app launches, taps/clicks, saved state, and other information about how a user interacts with the app. Lingua Relay conservatively includes the bounded internal routing record for an account-created live room under linked `Product Interaction`, used for App Functionality. The record contains only an internal non-invite room ID and its expiry, exists only so account deletion can close the room, and is removed no later than room expiry (24 hours after creation) or successful account deletion. The account's existing derived user ID remains separately declared as linked `User ID`.
...
- Retained account name, email, user ID, aggregate/recent usage, and the short-lived account-owned room-routing record are linked to the account and used for App Functionality only.
...
- Google defines `App interactions` as information about how a user interacts with the app. The bounded internal room ID/expiry record created when a signed-in account starts a room is therefore included as linked `App interactions`: collected, required for App functionality/account deletion, not shared, and not ephemeral because it remains until successful account deletion or the room's expiry, never longer than 24 hours. The record is an internal routing reference rather than a participant link, credential, peer identifier, or conversation record.
