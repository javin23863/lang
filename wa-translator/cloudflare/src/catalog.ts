...
export type LocaleProfile = {
  ...
  capabilities: Record<"asr" | "captions" | "tts", { available: boolean; reason: string }>;
...
const liveSpeech = new Set(document.release_live_speech_languages);
...
function profile(
  ...
  const voices = documentedVoices.filter(voice => releaseTtsProfileIds.has(voice.id))
    .map(voice => ({ ...voice }));
  const speech = liveSpeech.has(language.code);
  return {
    id, language: language.code, asr_code: language.code, mt_code: language.code,
    ...
    dialect_quality_claim: false,
    capabilities: {
      asr: { available: speech, reason: speech ? "" : "Not in the release-tested Whisper ∩ M2M100 live-speech set." },
...
export function publicCatalog(): Record<string, unknown> {
  ...
  return JSON.parse(JSON.stringify({
    ...
    release_live_speech_languages: [...liveSpeech].sort(),
    counts: {
      base_languages: document.languages.length,
      locale_profiles: locales.length,
      live_speech_languages: liveSpeech.size,
    ...
    languages: document.languages.map(language => ({
      code: language.code, mt_code: language.code, display_name: language.display_name,
